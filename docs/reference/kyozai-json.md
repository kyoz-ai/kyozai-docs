...
`build`を省略すると、`package.json`の`test`と`build`でartifactを作ります。`build.kind`を指定すると、kyoz.aiが用意したbuilderがartifactを作ります。現在は`jupyterlite`を指定できます。設定は[JupyterLite builder](/reference/jupyterlite-builder)を参照してください。
